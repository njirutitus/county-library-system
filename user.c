...
char* set_initials() {
    ...
    do {
        ...
        if(strcmp(initials,"") == 0) printf("Initials cannot be blank. Please try again.\n");
...
char* set_email()
{
    ...
    do {
        ...
        if(strcmp(email,"") == 0) printf("Email cannot be blank. Please try again.\n");
...
char* set_tel()
{
    ...
    do {
        ...
        valid = 1;
        if(strlen(tel)>13) {
            printf("Phone number can only contain up to 13 digits. Please try again.\n");
            valid = 0;
        }
        for(j=0;j<strlen(tel);j++){
            if(!isdigit(tel[j])) {
                valid = 0;
                printf("Phone number can only contain digits. Please try again.\n");
                break;
            }
        }
    } while(!valid);
...
int create_first_user() {
    char choice;
    printf("No users available. Would you like to create one? (Y/n): ");
    choice = getchar();
...
struct user get_updated_user()
{
    ...
    while(1) {
            do{
                ...
                scanf("%d",&attr);
                if(attr < 1 || attr > 9) {
                    printf("Invalid attribute. Please try again.\n");
                    Sleep(1000);
                    system("cls");
                }
            } while(attr < 1 || attr > 9);
            ...
            case 8:
                p = get_initial_password(u.name);
                strcpy(u.password,p);
                printf("Password was reset successfully! \n");
